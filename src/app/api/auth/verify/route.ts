...
export async function POST(request: Request) {
  try {
    ...
    // 2. Check if email belongs to any participant in registrations table
    ...
    if (regError) {
      console.error("Verify participant error:", regError);
    }

    if (regParticipant && regParticipant.teams) {
